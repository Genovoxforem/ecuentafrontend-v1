  ...
  return (
    <div className="relative" ref={boxRef}>
      ...
      {open && debounced.trim().length >= 2 && (
        <div className="absolute z-10 mt-1 w-full max-h-56 overflow-auto rounded-md border border-border bg-surface shadow-lg soft-scrollbar">
...
export function CreateProductModal({ taskCode, onClose, onCreated }: { taskCode: string; onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => {
    ...
    setUnits((cur) => cur || prefill.qtyUnit)
    setPacking((cur) => cur || prefill.packUnit)
  }, [prefill])
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-3 items-start">
            ...
            <Field label="Tags/categories">
              <select
                ...
                className={selectCls + ' h-auto min-h-9'}
              >
