...
import { ClipboardList, LoaderCircle, X, Check } from 'lucide-react'
...
export function InventoryCreateForm() {
  ...
  return (
    <StickyFormShell
      ...
      footerLeft={
        <Link to={ROUTES.inventoryList} className="flex items-center gap-1.5 rounded-lg border border-border px-4 py-2 text-sm font-medium text-text hover:bg-surface-hover">
          <X size={14} /> Cancel
        </Link>
      }
      footerRight={
        ...
        >
          {createInventory.isPending ? <LoaderCircle size={14} className="animate-spin" /> : <Check size={14} />} Create
        </button>
      }
    >
