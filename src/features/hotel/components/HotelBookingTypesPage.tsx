...
export function HotelBookingTypesPage() {
  ...
  function handleSave() {
    ...
    setSaveError('')
    save.mutate(
      { kind: 'booking', id: editing.id, name: editing.name, status: editing.status, token },
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="flex items-center gap-2 text-lg font-bold text-text!">
          <CalendarCheck size={20} className="text-brand" /> Booking Types
        </h2>
        <button
          type="button"
      ...
      <Card className="!h-auto flex items-start gap-2 bg-info-bg/40">
        ...
        <p className="text-xs text-info-fg">
          Backend page: <code className="font-mono">booking/settings/booking_types.php</code> — real and working. This uses the Hotel Suite API instead
          (same underlying data, confirmed live), which is what the rest of this app's booking source options read from.
        </p>
      </Card>
      ...
      {editing && (
        <Card className="!h-auto">
          <div className="flex items-center gap-2">
            <input value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} placeholder="Booking type name" className={`flex-1 ${fieldCls}`} />
            <select
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-text-faint uppercase tracking-wide border-b border-border bg-surface">
                ...
                <th className="font-medium px-4 py-2.5">Booking Type</th>
                  ...
                  <tr key={r.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-2.5">
                      <button
                        type="button"
                        disabled={!token}
                        onClick={() => token && save.mutate({ kind: 'booking', id: r.id, name: r.name, status: r.status === 1 ? 2 : 1, token })}
