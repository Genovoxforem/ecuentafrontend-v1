...
export function InvoiceStatisticsPage() {
  const { data } = useInvoicesSummary()
  const [thirdParty, setThirdParty] = useState('')
  const [year, setYear] = useState(String(new Date().getFullYear()))

  const rows = useMemo(() => data?.rows ?? [], [data])
  const thirdPartyOptions = useMemo(() => Array.from(new Set(rows.map((r) => r.thirdParty).filter(Boolean))).map((n) => ({ value: n, label: n })), [rows])
  const filteredRows = useMemo(() => (thirdParty ? rows.filter((r) => r.thirdParty === thirdParty) : rows), [rows, thirdParty])
  ...
  const byMonthAmount = useMemo(() => {
    ...
    for (const r of filteredRows) {
      if (r.invoiceDate?.slice(0, 4) === year) amounts[new Date(r.invoiceDate).getMonth()] += r.amountInclTax
    }
    return amounts
  }, [filteredRows, year])
