...
import { Mail, X, LoaderCircle, Paperclip, Check } from 'lucide-react'
import { usePurchaseOrderEmailDefaults, useSendPurchaseOrderEmail } from '../purchaseOrderEmail.queries'
...
// Native replacement for linking out to fourn/commande/card.php?...&action=presend
// — see purchaseOrderEmail.queries.ts for why this POSTs straight to that
// same real backend endpoint (Dolibarr's own stock send-mail action, same
// mechanism as Sales Order's SendOrderEmailModal).
export function SendPurchaseOrderEmailModal({ id, orderRef, onClose }: { id: string; orderRef: string; onClose: () => void }) {
  const { data, isLoading, isError, error, refetch } = usePurchaseOrderEmailDefaults(id, true)
      ...
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-lg bg-surface border border-border p-5 shadow-xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="flex items-center gap-2 text-sm font-semibold text-text!">
            <Mail size={16} className="text-brand" /> Send email — {orderRef}
          </h3>
            ...
            <div className="flex flex-col gap-1">
              ...
              {data.attachedFileName && (
                <span className="flex items-center gap-1.5 text-xs text-text-muted">
                  <Paperclip size={12} /> {data.attachedFileName} (already attached)
                </span>
              )}
              <input
                type="file"
