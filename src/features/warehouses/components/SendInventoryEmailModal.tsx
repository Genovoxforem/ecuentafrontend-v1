...
import { useInventoryEmailDefaults, useSendInventoryEmail } from '../inventoryEmail.queries'
...
// Native replacement for linking out to product/inventory/card.php?...
// &action=presend — same real Dolibarr stock send-mail action as
// SendOrderEmailModal.tsx, adapted for Inventory's own real field values
// (models="inventory", trackid="stockinv"+id — see inventoryEmail.ts).
export function SendInventoryEmailModal({ id, inventoryLabel, onClose }: { id: string; inventoryLabel: string; onClose: () => void }) {
  const { data, isLoading, isError, error, refetch } = useInventoryEmailDefaults(id, true)
  const sendEmail = useSendInventoryEmail()
  ...
  function handleSubmit() {
    ...
    if (!data) return
    sendEmail.mutate({ id, token: data.token, returnUrl: data.returnUrl, fromtype, sendto, sendtocc, subject, message })
  }
      ...
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-lg bg-surface border border-border p-5 shadow-xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="flex items-center gap-2 text-sm font-semibold text-text!">
            <Mail size={16} className="text-brand" /> Send email — {inventoryLabel}
          </h3>
