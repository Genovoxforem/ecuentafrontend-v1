...
type SortKey = 'ref' | 'paymentDate' | 'thirdPartyName' | 'paymentTypeLabel' | 'amount'
...
function matchesSearch(r: PurchasePaymentRow, query: string) {
  ...
  if (!q) return true
  return [r.ref, r.thirdPartyName ?? '', r.paymentTypeLabel ?? ''].some((field) => field.toLowerCase().includes(q))
}
...
export function VendorPaymentsReportPage() {
  ...
  function getExportData() {
    const exportRows = sorted.map((r) => [r.ref, formatDateTimeAmPm(r.paymentDate), r.thirdPartyName || '-', r.paymentTypeLabel || '-', formatMoney(r.amount)])
      ...
      <div className="flex-1 flex flex-col min-h-0 space-y-4 px-6 py-4">
        ...
        <Card className="!p-0 overflow-hidden flex-1 min-h-0">
          <div className="flex flex-wrap items-center gap-3 p-4 border-b border-border">
            ...
            <TableExportButtons title={`Vendor Payments Report ${MONTHS[applied.month]} ${applied.year}`} getExportData={getExportData} />
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={COLUMN_LABELS.length} className="px-4 py-4 text-text-faint italic">
                      {periodFiltered.length === 0 ? 'No Data Available In Table' : `No vendor payments match "${search}".`}
                    </td>
                  </tr>
                ) : (
                  pageRows.map((r) => (
